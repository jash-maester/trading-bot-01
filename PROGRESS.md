...
| R4 — supervised cross-sectional | **PASS on rank IC; FAIL on top-K** (`audit/S6`) | Window-level: mean of per-window OOS rank IC > 0.02, window t > t_crit(95%), >= 75% windows positive (`12_gate_decision.md`) | `data/signal/r4_v2/gate.json`: 5d +0.0392 t 7.50, 20d +0.0437 t 5.31, 8/8 windows positive | 2026-09-06 |
| R4-topk — what the allocator consumes | **FAIL as a picker; short-leg information** | Top-30 20d forward return net of cost vs 20 random books, per window (`scripts/topk_gate.py`) | `r4_pit_long` 13 win: top-30 rank **12/21** (indistinguishable from random); bottom-30 **−0.85%/20d, t −3.20, rank 21/21** — worse than every random book. R4's IC lives in the tail a long-only book never buys: it is a screen, not a picker. `audit/S6_TOPK_GATE.md`. | 2026-09-10 |
| R4b — cross-sectional inputs | **RUN — FAIL, not adopted** | Beat `r4_pit_long`'s window-level gate | `train.xs_normalise=rank` at full parity (`r4_pit_xs`). WORSE on both spans: 20d gate +0.0280 (t 4.19, PASS) → **+0.0131 (t 1.52, FAIL)**; unseen holdout +0.0459 → +0.0117, 3 of 15 months favouring it. The motivating gap — one ranked feature scoring 2× the model — is **in-sample only and reverses out of sample**. Likely mechanism: ranking each day destroys the time axis the TCN reads. `audit/S3_CROSS_SECTIONAL_INPUTS.md`. | 2026-09-09 |
| R5-control — matched null | **RUN — SIGNAL REFUTED vs random** | Beat a random signal running identical machinery | Same universe/K/band/stop/costs/tax, random scores: R4 wins **8 of 8** matched pairs in sample, loses **13 of 15** out of sample (sign test p≈0.004). R5's +0.0431/yr over equal-weight is ~+0.0261 machinery, ~+0.017 signal, and the signal part does not survive. The pre-2026-09-09 control was broken (band 0.0, no overlay, no NAV) so every earlier risk comparison was uncontrolled. `audit/S4_NULL_CONTROL.md`. | 2026-09-09 |
| R5-selection — turnover rule | **RUN — FAIL, no forward test warranted** | Implement S4's surviving machinery with a non-random selection | Top-30 by 20d median turnover, pre-committed, K=30 band 0.010 monthly: **worse than random in 8 of 8 arms** on both spans; holdout loses 4.5–7.6%/yr absolute. The effect was breadth across the liquidity spectrum, not concentration — mega-caps cut drawdown by giving up the return that was working. Out of sample all that survives is a vol stop cutting drawdown at no return gain. `audit/S5_TURNOVER_SELECTION.md`. | 2026-09-09 |
...
| 7 | Forward permutation test (`audit/P2`) | **LIVE ON DOCKER — restart 2, record from 2026-09-25** — compose service `paper` (supercronic, 21:00 + 07:30 IST); host cron retired (TCC never let it run) | One signal book vs 20 random books, K=30 band 0.010 monthly volstop, Kite-free, on the Mac via launchd (21:00 + 07:30 IST). Run of record 2026-09-10T11:05:43Z; warm-up rank **17/21**; determinism 3.3e-11. Verdict at 48 months only. `scripts/paper_daily.sh`, `audit/paper/record.jsonl`. |
